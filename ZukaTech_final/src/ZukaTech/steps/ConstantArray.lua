...
function ConstantArray:apply(ast, pipeline)
	...
	-- Inject fake (unreferenced) entries scattered through the array to bloat
	-- and confuse static analysis. Real indices are adjusted to skip fakes.
	do
		local fakeCount = math.random(3, 8);
		local fakePool = {
			"__index","__newindex","__tostring","__len","__concat",
			"rawget","rawset","ipairs","pairs","select",
			"setmetatable","getmetatable","type","tonumber","tostring",
		};
		local realSet = {};
		for _, v in ipairs(self.constants) do realSet[tostring(v)] = true end

		for _ = 1, fakeCount do
			local fakeVal;
			local attempts = 0;
			repeat
				if math.random(2) == 1 then
					...
					fakeVal = math.random(1, 9999);
				end
				attempts = attempts + 1;
			until not realSet[tostring(fakeVal)] or attempts > 20;

			if not realSet[tostring(fakeVal)] then
				local pos = math.random(1, #self.constants + 1);
				table.insert(self.constants, pos, fakeVal);
				local newLookup = {};
				for k, idx in pairs(self.lookup) do
					if idx >= pos then
						newLookup[k] = idx + 1;
					else
						newLookup[k] = idx;
