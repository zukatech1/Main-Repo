...
function Compiler:createBlock()
    local id;
    repeat
        -- Apply per-compile salt so the same source produces different IDs each run.
        -- The salt is added then masked to 20 bits to stay within a safe range.
        local raw = math.random(0, 2^20);
        id = (raw + (self.blockIdSalt or 0)) % (2^20) + 1;
...
function Compiler:compile(ast)
    ...
    -- Per-compile ID salt: all block IDs are offset by a random base so the
    -- same source never produces the same ID layout across compilations.
    -- This makes it impossible to build a static ID-to-opcode mapping.
    self.blockIdSalt = math.random(0, 2^18);
...
function Compiler:emitContainerFuncBody()
    ...
    -- NOP block injection: sprinkle dead blocks with IDs between real blocks.
    -- Each NOP block unconditionally jumps to its nearest real neighbour, so
    -- the VM dispatcher gains extra branches that are never actually taken for
    -- any real instruction sequence. This makes static block-ID mapping harder.
    do
        local nopCount = math.random(2, 5);
        for _ = 1, nopCount do
            if #blocks >= 2 then
                -- Pick two adjacent real blocks and insert a NOP with an ID between them
                local pos = math.random(1, #blocks - 1);
                local idA  = blocks[pos].id;
                local idB  = blocks[pos + 1].id;
                if idB > idA + 1 then
                    local nopId = math.random(idA + 1, idB - 1);
                    if not self.usedBlockIds[nopId] then
                        self.usedBlockIds[nopId] = true;
                        -- NOP body: junk local assignment then jump to idB (the next real block).
                        -- We reuse containerFuncScope for the nop scope.
                        local nopScope = Scope:new(self.containerFuncScope);
                        local junkVar  = nopScope:addVariable();
                        local c1 = math.random(1, 255);
                        local c2 = math.random(1, 255);
                        local junkStat = Ast.LocalVariableDeclaration(nopScope, {junkVar}, {
                            Ast.AddExpression(Ast.NumberExpression(c1), Ast.NumberExpression(c2))
                        });
                        nopScope:addReferenceToHigherScope(self.containerFuncScope, self.posVar);
                        local jumpStat = Ast.AssignmentStatement(
                            {Ast.AssignmentVariable(self.containerFuncScope, self.posVar)},
                            {Ast.NumberExpression(idB)}
                        );
                        local nopBlock = Ast.Block({junkStat, jumpStat}, nopScope);
                        table.insert(blocks, pos + 1, {id = nopId, block = nopBlock});
                    end
                end
            end
        end
